...
class RvalueOperatorTest : public testing::Test {
 public:
  struct F {
    explicit F(int* calls) : calls(calls) {}
    void operator()() && {
      if (calls) (*calls)++;
    }
    int* calls = nullptr;
...
TEST_F(RvalueOperatorTest, InvokesRvalueOperatorInDestructor) {
  { auto c = MakeCleanup(F(&calls_)); }
  EXPECT_EQ(calls_, 1);
}

TEST_F(RvalueOperatorTest, InvokesRvalueOperatorInAssignment) {
  {
    auto c = MakeCleanup(F(&calls_));
    c = MakeCleanup(F(nullptr));
    EXPECT_EQ(calls_, 1);
  }
  EXPECT_EQ(calls_, 1);
}
