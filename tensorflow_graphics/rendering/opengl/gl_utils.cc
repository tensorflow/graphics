...
tensorflow::Status Program::CompileShader(const std::string& shader_code,
  ...
  if (*shader_idx == 0)
    return TFG_INVALID_ARGUMENT("Error while creating the shader object.");
  auto shader_cleanup =
  ...
  if (compilation_status != GL_TRUE) {
    ...
    return TFG_INVALID_ARGUMENT("Error while compiling the shader: " +
...
tensorflow::Status Program::Create(
  ...
  if (program_handle == 0)
    return TFG_INVALID_ARGUMENT("Error while creating the program object.");
  ...
  for (auto shader : shaders) {
    ...
    TFG_RETURN_IF_TF_ERROR(
        CompileShader(shader.first, shader.second, &shader_idx));
...
tensorflow::Status Program::GetResourceProperty(
  ...
  if (num_property_value != num_properties)
    return TFG_INVALID_ARGUMENT("num_property_value != num_properties");

  GLuint resource_index;
  // Query the index of the named resource within the program.
  TFG_RETURN_IF_TF_ERROR(GetProgramResourceIndex(
  ...
  GLsizei length;
  TFG_RETURN_IF_TF_ERROR(GetProgramResourceiv(
