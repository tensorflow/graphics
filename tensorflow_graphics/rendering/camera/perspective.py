...
def intrinsics_from_matrix(matrix, name=None):
  ...
  Returns:
    Tuple of three tensors, first two of shape `[A1, ..., An, 2]` and the third of shape `[A1, ..., An, 1]`. The first
    tensor represents the focal length, and the second one the principle point. The third represent Skew Coefficient.

  Raises:
    ValueError: If the shape of `matrix` is not supported.
  """
  with tf.compat.v1.name_scope(name, "perspective_intrinsics_from_matrix", [matrix]):
      ...
      cx = matrix[..., 0, 2]
      cy = matrix[..., 1, 2]
      skew_coefficient = matrix[..., 0, 1]
      focal = tf.stack((fx, fy), axis=-1)
      principal_point = tf.stack((cx, cy), axis=-1)
      skew_coefficient = tf.expand_dims(skew_coefficient, axis=0)

  return focal, principal_point, skew_coefficient


def matrix_from_intrinsics(focal, principal_point, skew_coefficient=(0.0,), name=None):
  ...
  Args:
    ...
    skew_coefficient: A tensor of shape `[A1, ..., An, 1]`, where the last
      dimension represents a skew coefficient.
    name: A name for this op that defaults to
  ...
  """
  with tf.compat.v1.name_scope(
      name, "perspective_matrix_from_intrinsics", [focal, principal_point]
  ):
      focal = tf.convert_to_tensor(value=focal)
      principal_point = tf.convert_to_tensor(value=principal_point)
      skew_coefficient = tf.convert_to_tensor(value=skew_coefficient)
      ...
      skew_coefficient_shape = skew_coefficient.shape.as_list()
      if None not in skew_coefficient_shape:
          skew_coefficient_shape.pop()
      ...
      skew_coefficient = tf.reshape(skew_coefficient, tf.shape(fx))

      matrix = tf.stack(
          (fx, skew_coefficient, cx, zero, fy, cy, zero, zero, one), axis=-1
      )  # pyformat: disable
      matrix_shape = tf.shape(input=matrix)
...
def project(point_3d, focal, principal_point, name=None):
  ...
  """
  with tf.compat.v1.name_scope(
      name, "perspective_project", [point_3d, focal, principal_point]
  ):
...
def ray(point_2d, focal, principal_point, name=None):
  ...
  """
  with tf.compat.v1.name_scope(
      name, "perspective_ray", [point_2d, focal, principal_point]
  ):
...
def unproject(point_2d, depth, focal, principal_point, name=None):
  ...
  """
  with tf.compat.v1.name_scope(
      name, "perspective_unproject", [point_2d, depth, focal, principal_point]
  ):
      point_2d = tf.convert_to_tensor(value=point_2d)
      ...
      shape.check_static(tensor=depth, tensor_name="depth", has_dim_equals=(-1, 1))
      shape.check_static(tensor=focal, tensor_name="focal", has_dim_equals=(-1, 2))
      shape.check_static(
          tensor=principal_point,
          tensor_name="principal_point",
