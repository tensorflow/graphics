...
@tf.function
def get_distances_between_points(ray_points3d):
  ...
  return tf.concat([dists, tf.broadcast_to([1e10], dists[..., :1].shape)],
                   axis=-1)
...
def camera_rays_from_extrinsics(rays,
                                rotation_matrix,
                                translation_vector):
  """Estimate ray origin and direction from camera extrinsics.
