...
def compute_uvs(vertices, indices, print_progress=True):
    ...
    with tf.device('/device:cpu:' + str(pyredner.get_cpu_device_id())):
        ...
        uv_trimesh = pyredner.UVTriMesh(pyredner.float_ptr(pyredner.data_ptr(vertices)),
                                        pyredner.int_ptr(pyredner.data_ptr(indices)),
                                        pyredner.float_ptr(0),
                                        pyredner.int_ptr(0),
                                      int(vertices.shape[0]),
                                      0,
                                      int(indices.shape[0]))

        atlas = pyredner.TextureAtlas()
        num_uv_vertices = pyredner.automatic_uv_map([uv_trimesh], atlas, print_progress)[0]

        uvs = tf.zeros(num_uv_vertices, 2, dtype=tf.float32)
        uv_indices = tf.zeros_like(indices)
        uv_trimesh.uvs = pyredner.float_ptr(pyredner.data_ptr(uvs))
        uv_trimesh.uv_indices = pyredner.int_ptr(pyredner.data_ptr(uv_indices))
        uv_trimesh.num_uv_vertices = num_uv_vertices

        pyredner.copy_texture_atlas(atlas, [uv_trimesh])
